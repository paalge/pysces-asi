...
def create_quicklook(image, output, settings_manager):   
    settings_manager.set({'output':"OutputTaskHandler> Creating quicklook for "+image.getFilename()})
...
def centered_image(image, output, settings_manager):
    settings_manager.set({'output':"OutputTaskHandler> Creating centered image for "+image.getFilename()})
    im = image.binaryMask(output.fov_angle)
    im = im.centerImage()
    im = im.alignNorth(north="geomagnetic", orientation='NWSE')
    return im 
...
def check_keo_compatibility(keo, image, output):
    ...
    keo_time_width_hours = (keo_time_width.days * 24) + (keo_time_width.seconds/3600.0)
    ...
    if ((image.getMode() != keo.getMode()) or
        ...
        #don't need to compare the image fov angle, only that set for the keogram
        return False
    return True
...
def realtime_keogram(image, output, settings_manager):
    ...
    if filename is None or not os.path.exists(filename):
        settings_manager.set({'output':"OutputTaskHandler> Creating new realtime keogram."})
        ...
        try:
            ...
            settings_manager.set({'user_rt_keo_name': os.path.expanduser('~')+"/realtime_keogram"})
    else:
        keo = allskyKeo.load(filename)
        
        settings_manager.set({'output':"OutputTaskHandler> Adding image to realtime keogram."})
        ...
        if check_keo_compatibility(keo, image, output):        
            ...
            #some of the settings must have changed - time to start a new keogram
            settings_manager.set({'user_rt_keo_name':None,'output':"OutputTaskHandler> New image is not compatible with existing keogram."})
