...
class NetworkManager(ThreadQueueBase):
    ...
    def _mount_server(self):
        ...
        if ((not self._is_mounted()) and (not self.__awaiting_password)):
            ...
            os.chmod(home+"/.pysces_asi/mount-script.sh", stat.S_IRWXU+stat.S_IRWXO+stat.S_IRWXG)
            
            #run script in xterm window
            p = Popen("xterm -e "+home+"/.pysces_asi/mount-script.sh", shell=True, stdout=PIPE, stderr=PIPE)
