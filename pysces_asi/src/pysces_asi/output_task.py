...
def create_output_tasks(capture_mode, image_files, folder_on_host, settings_manager):
    ...
    for image_type in list(image_files.keys()):
        ...
        output_tasks.append(
            OutputTask(outputs, image_files[image_type], folder_on_host, settings_manager))
...
class OutputTask:
    ...
    def run_subtasks(self, processing_pool, pipelined_processing_pool, network_manager):
        ...
        for output in self._outputs:
            ...
            sub_task = SubTask(
                function, self._image_file, output, self._folder_on_host)

            # if we need to copy files to the web-server, then create a proxy
    ...
    def wait(self):
            ...
            try:
                ...
                if safe_delete:
                    ...
                    self._settings_manager.set(
                        {'output': "OutputTask> Error! Processing pool failed to execute one or more sub-tasks"})
                    self._settings_manager.set(
                        {'output': "OutputTask> Leaving temporary files in place"})
                else:
                    self.__remove_files = True
                    self._settings_manager.set(
                        {'output': "OutputTask> Error! Processing pool failed to execute one or more sub-tasks"})
                    self._settings_manager.set(
                        {'output': "OutputTask> Safe delete is off, removing temporary files anyway."})
        ...
        try:
            ...
            # if there are any syncronisation problems then just give up - it's
            # not that important
            pass
